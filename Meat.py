...
class Meat(QWidget):
    ...
    def initUI(self):
        ...
        root.addWidget(self.meattable)
        root.addLayout(bottom)
        self.setLayout(root)
        self.setWindowTitle("Meat")
    ...
    def populate(self):
        ...
        meattable = QTableWidget()
        meattable.setRowCount(self.getRowCount())
        meattable.setColumnCount(6)
        meattable.setHorizontalHeaderLabels(['Quantity','Item Name', 'Description','Expiration Date','Price','In Stock'])
        ...
        for i in range(self.getRowCount()):
            ...
            spinBox = QSpinBox()
            meattable.setCellWidget(i, 0, spinBox)
            ...
            for j in range(5):
                ...
                if j == 0:
                    ...
                    meattable.setItem(i, 1, itemName)
                ...
                if j == 1:
                    ...
                    meattable.setItem(i, 2, description)
                ...
                if j == 2:
                    ...
                    meattable.setItem(i, 3, quantity)
                ...
                if j == 3:
                    ...
                    meattable.setItem(i, 4, retailPrice)
                ...
                if j == 4:
                    ...
                    meattable.setItem(i, 6, expDate)

        return meattable

    def getRowCount(self):
        cursor = self.connection.cursor()
        numofRows = 'select count(*) from item, soldat where item.item_id=soldat.item_id and food_group="Meat" and soldat.store_id={};'.format(self.store_id)
    ...
    def getItemIDs(self):
        ...
        select = ('SELECT item.item_id FROM ITEM, SOLDAT WHERE item.item_id = soldat.item_id and food_group = "Meat" and soldat.store_id={};'.format(self.store_id))
    ...
    def getExpDate(self, item_id):
        ...
        select = ('SELECT item.exp_date FROM ITEM WHERE food_group = "Meat" and item_id={};'.format(item_id))
        cursor.execute(select)
        return cursor.fetchone()[0]
    ...
    def getItemName(self, item_id):
        ...
        select = ('SELECT item.item_name FROM ITEM WHERE food_group = "Meat" and item_id={};'.format(item_id))
        cursor.execute(select)
        return cursor.fetchone()[0]
    ...
    def getRetailPrice(self, item_id):
        ...
        select = ('SELECT item.listed_price FROM ITEM WHERE food_group = "Meat" and item_id={};'.format(item_id))
        cursor.execute(select)
        return cursor.fetchone()[0]
    ...
    def getDescription(self, item_id):
        ...
        select = ('SELECT item.description FROM ITEM WHERE food_group = "Meat" and item_id={};'.format(item_id))
        cursor.execute(select)
        return cursor.fetchone()[0]
    ...
    def getQuantity(self, item_id):
        ...
        select = ('SELECT item.quantity FROM ITEM WHERE food_group = "Meat" and item_id={};'.format(item_id))
        cursor.execute(select)
        ...
        #help populate Meat
        #select grocerystore.store_id, store_name, item_name from item, grocerystore, soldat where food_group="Meat" and soldat.store_id=grocerystore.store_id and item.item_id=soldat.item_id;
